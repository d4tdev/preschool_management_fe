...
class AddMealForm extends Component {
    ...
    onSubmit = (event) => {
        ...
        // Validation cơ bản
        if (!name) {
            this.setState({ error: 'Vui lòng điền tên bữa ăn.' });
            return;
        }
        ...
        CallApi('meal', 'POST', {
            ...
            .then((res) => {
                if (res.status === 200 || res.status === 201) {
                    ...
                    // Có thể chuyển hướng về danh sách bữa ăn nếu cần
                    // this.props.history.push('/home/list-meals');
                } else {
                    this.setState({
                        error: 'Thêm bữa ăn thất bại. Vui lòng thử lại.',
                    });
                }
            })
            .catch((err) => {
